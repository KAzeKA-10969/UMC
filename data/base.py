...
def get_data(args, logger):
    ...
    logger.info(f"data_path: {data_path}")  # 调试信息
    ...
    train_file_path = os.path.join(data_path, 'train.tsv')
    logger.info(f"train_file_path: {train_file_path}")  # 调试信息
    
    train_data_index, train_label_ids = get_indexes_annotations(args, bm, label_list, train_file_path)
    
    dev_file_path = os.path.join(data_path, 'dev.tsv')
    logger.info(f"dev_file_path: {dev_file_path}")  # 调试信息
    
    dev_data_index, dev_label_ids = get_indexes_annotations(args, bm, label_list, dev_file_path)
    
    train_data_index = train_data_index + dev_data_index
    train_label_ids = train_label_ids + dev_label_ids

    test_file_path = os.path.join(data_path, 'test.tsv')
    logger.info(f"test_file_path: {test_file_path}")  # 调试信息
    
    test_data_index, test_label_ids = get_indexes_annotations(args, bm, label_list, test_file_path)
